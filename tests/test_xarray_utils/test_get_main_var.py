...
CMIP5_FPATHS = [
    ...
    CMIP5_ARCHIVE_BASE
    + "/cmip5/output1/MOHC/HadGEM2-ES/rcp85/mon/atmos/Amon/r1i1p1/latest/tas/*.nc", #cmip5_tas
    CMIP5_ARCHIVE_BASE
...
def test_get_main_var_test_data(cmip5_tas):
    ds = xr.open_mfdataset(cmip5_tas, use_cftime=True, combine="by_coords")
    var_id = get_main_variable(ds)
    assert var_id == "tas"


def test_get_main_var_include_common_coords(cmip5_tas):
    ds = xr.open_mfdataset(cmip5_tas, use_cftime=True, combine="by_coords")
