# NOTE: This has been largely copied over from CLISOPS with the intention to eventually merge the two files
import os
...
from roocs_utils.utils import testing
from roocs_utils.utils.testing import stratus as _stratus
...
def write_roocs_cfg(stratus, tmp_path) -> str:
    ...
    """
    roocs_config = tmp_path.joinpath("roocs.ini")
    cfg = Template(cfg_template).render(base_dir=stratus.path)
    with roocs_config.open("w") as fp:
        fp.write(cfg)
