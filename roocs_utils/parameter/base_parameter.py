    ...
    def _parse_range(self):
        if self.input in ('/', None, ''):
            ...
            if '/' not in self.input:
                raise InvalidParameterValue(f"{self.__class__.__name__} should be passed in as a range separated by /")
            ...
            if len(self.input) != 2:
                raise InvalidParameterValue(f"{self.__class__.__name__} should be a range. Expected 2 values, "
                                            f"received {len(self.input)}")

            start, end = self.input

        else:
            raise InvalidParameterValue(f"{self.__class__.__name__} is not in an accepted format")
        return start, end
